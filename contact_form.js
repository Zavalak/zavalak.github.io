export const initContactForm = () => {
  ...
  // Replace this with your actual SheetDB API URL from Step 2
  const SHEETDB_API_URL = "PASTE_YOUR_SHEETDB_API_URL_HERE";
  ...
  const renderForm = () => {
      ...
      <div id="form-inner">
        ...
        <form id="hvac-contact-form" class="space-y-4">
          <div class="grid md:grid-cols-2 gap-4">
            <input type="text" id="name" name="name" placeholder="FULL NAME" required class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all uppercase font-bold" />
            <input type="email" id="email" name="email" placeholder="EMAIL ADDRESS" required class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all uppercase font-bold" />
          </div>

          <div class="grid md:grid-cols-2 gap-4">
            <input type="tel" id="phone" name="phone" placeholder="PHONE (10 DIGITS)" required maxlength="10" pattern="[0-9]{10}" class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all uppercase font-bold" />
            <input type="text" id="extension" name="extension" placeholder="EXT. (OPTIONAL)" class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all uppercase font-bold" />
          </div>

          <div class="grid md:grid-cols-2 gap-4">
            <select id="category-select" name="category" required class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all appearance-none cursor-pointer uppercase font-bold">
              <option value="" disabled selected>SELECT CATEGORY</option>
              ${Object.keys(issueData).map(cat => `<option value="${cat}">${cat.toUpperCase()}</option>`).join('')}
            </select>
            
            <select id="reason-select" name="reason" required disabled class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all appearance-none cursor-pointer opacity-50 uppercase font-bold">
              <option value="" disabled selected>SELECT SPECIFIC REASON</option>
            </select>
          </div>

          <textarea id="requirements" name="details" placeholder="ADDITIONAL DETAILS OR COMMENTS" rows="4" class="w-full bg-slate-900 border border-slate-800 p-5 text-xs tracking-widest text-white focus:border-[#B87333] outline-none transition-all uppercase font-bold"></textarea>
    ...
    form.addEventListener('submit', (e) => {
      ...
      .then(response => {
        if (response.ok) {
          renderSuccess(); // Triggers GSAP success animation
        } else {
      ...
      container.innerHTML = `
        <div class="text-center py-20 border border-[#B87333]/30 bg-slate-900/20" id="success-msg" style="opacity: 0">
          ...
          <div class="pt-8 border-t border-slate-800">
            <p class="text-[10px] tracking-widest text-slate-500 mb-2 font-bold uppercase">Urgent Assistance</p>
